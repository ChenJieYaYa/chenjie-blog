...
块级元素独占一行，在网页中一般通过块元素来对页面进行布局，此处只介绍部分

|                      标签                      |  作用  |                             描述                             |                             图解                             |
| :--------------------------------------------: | :----: | :----------------------------------------------------------: | :----------------------------------------------------------: |
| `<h1>`、`<h2>`、`<h3>`、`<h4>`、`<h5>`、`<h6>` |  标题  | 一共六级标题，从`h1~h6`重要性递减，`h1`在网页中的重要性仅次于`title`标签，一般情况下一个页面中只会有一个`h1` |          ![1669036578476](assets/1669036578476.png)          |
|                   `<hgroup>`                   | 标题组 |          多层次的标题，它将一组`<h1>~<h6>`元素分组           | <h1>标题一</h1><h2>标题二</h2><h3>标题三</h3><h4>标题四</h4><h5>标题五</h5><h6>标题六</h6> |
|                     `<p>`                      |  段落  |                                                              |          ![1669036559194](assets/1669036559194.png)          |
|                 `<blockquote>`                 | 长引文 | 缩进后显示文本，`cite`属性表示引文来源，`<cite>`元素表示来源的文本表述 |          ![1669036602592](assets/1669036602592.png)          |

### 2.内联元素

内联元素又称行内元素，行内元素主要用来包裹文字，一般情况下会在块级元素中放行内元素，而不会在行内元素中放块级元素，此处只介绍部分

|    标签    |  作用  |                             描述                             |                    图解                    |
| :--------: | :----: | :----------------------------------------------------------: | :----------------------------------------: |
|   `<q>`    | 短引文 |                       文本被双引号包裹                       | ![1669036652578](assets/1669036652578.png) |
|   `<br>`   |  换行  |                                                              |                                            |
|   `<em>`   |  强调  | `<em>`元素可以嵌套，每一级嵌套表示更高的强调程度，`<i>`元素效果与它相同，不过`<i>`不属于语义标签 | ![1669036876849](assets/1669036876849.png) |
| `<strong>` |  重要  | 表示重要性、严肃性或紧迫性，浏览器通常以粗体字呈现内容，`<b>`元素效果与它相同，不过`<b>`不属于语义标签 | ![1669036892653](assets/1669036892653.png) |
