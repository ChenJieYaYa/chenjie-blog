...
前两种方案都无法获得新线程的执行结果，FutureTask继承结构如下图，由图可知`call()`返回结果，**将在线程并发工具类中详细讲解**
...
`synchronized`用于解决多线程操作同一资源时数据不一致问题，即**资源争抢问题**，`synchronized`有三种使用方式，即同步代码块、同步方法(对象锁)、同步静态方法(类锁)，注意构造方法不能加锁，在线程高级部分具体讲解
