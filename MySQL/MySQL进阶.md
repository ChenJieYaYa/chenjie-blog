...
> SQL优化中索引很重要，已经在索引6和7中详细介绍

### 1.插入数据

#### 1.1.insert优化

①**批量插入**：批量插入可节省每条插入语句建立连接的时间，注意一次性插入的数据不建议超过一千条

```mysql
insert into table_name values(1,'name1'),(2,'name2');
```

②**手动事务提交**：每插入一次就需要开启一次事务，导致频繁的事务开启

```mysql
start transation;
insert into table_name values(1,'name1'),(2,'name2');
insert into table_name values(3,'name3'),(4,'name4');
insert into table_name values(5,'name5'),(6,'name6');
commit;
```
...
一次插入大批量数据使用`insert`性能较低，此时使用`load`指令进行插入，**直接将本地磁盘文件解析成数据**

![1668435877962](assets/1668435877962.png)

使用`load`前还需要执行以下两个步骤
...
```mysql
load data local infile '数据磁盘文件路径' into table `table_name` fields terminated by ',' lines terminated '\n';
```
...
前面我们提到根据主键顺序插入性能更佳，那么为什么呢？

InnoDB中表数据根据主键形成聚集索引存储，而聚集索引中**每个节点都是一页**，页大小固定为16K，插入数据时会先申请一页，等这一页存不下才会继续申请第二页，**每页之间维护一个双向指针**，若是主键顺序插入则如下图

![1668437040955](assets/1668437040955.png)

但主键乱序插入则会发生**页分裂**，页分裂过程如下图

![1668437256850](assets/1668437256850.png)

![1668437347514](assets/1668437347514.png)

![1668437567251](assets/1668437567251.png)

![1668437607079](assets/1668437607079.png)

![1668437664761](assets/1668437664761.png)

既然存在页分裂，那么必然也存在**页合并**，页合并过程如下图

![1668437929195](assets/1668437929195.png)

![1668438085825](assets/1668438085825.png)

![1668438152732](assets/1668438152732.png)

#### 2.2.主键设计原则

①**尽量降低主键长度**：二级索引叶子节点数据就是主键，若二级索引较多会占用大量磁盘空间，搜索时耗费大量磁盘IO

②**插入数据时尽量顺序插入，原则主键自增`AUTO_INCREMENT`👀**：主键乱序插入容易发生页分裂

③**不要使用UUID或身份证作为主键**：UUID无序，身份证过长
